...
| 序号 | 题目 | 难度 | 说明 | 是否完成 |
|:----:|------|:----:|------|:-------:|
| 739 | 每日温度 | 中等 | 下一个更高温度的天数差 | ✓ |
| 84 | 柱状图中最大的矩形 | 困难 | 单调栈求最大矩形面积 | ✓ |
| 85 | 最大矩形 | 困难 | 每层柱状图+单调栈求最大矩形 | ✓ |
| 42 | 接雨水 | 困难 | 单调栈/双指针求接水量 | ✓ |
| 496 | 下一个更大元素 I | 简单 | 单调栈求数组中NEXT更大元素 | ✗ |
| 503 | 下一个更大元素 II | 中等 | 循环数组的下一个更大元素 | ✗ |
| 402 | 移掉 K 位数字 | 中等 | 单调栈+贪心使移除后数字最小 | ✓ |
| 316 | 去除重复字母 | 中等 | 单调栈保持字典序最小去重 | ✗ |
| 901 | 股票价格跨度 | 中等 | 单调栈维护最近不超过价格的天数 | ✗ |
| 321 | 拼接最大数 | 困难 | 单调栈+归并列选最大组合数 | ✓ |
